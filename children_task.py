...
def text2idx(text):
    return [VOC_SIZE] + [DICTIONARY.index(c) for c in unicodedata.normalize('NFD', text) if c in DICTIONARY[1:-1]] + [
        VOC_SIZE + 1]
...
#
# Pytorch Model Codes
# reference : README 참조, code repo 2개, 논문 1개, 노하우 추가.
#
# Attention Module
...
class BatchRNN(nn.Module):
    def __init__(self, input_size, hidden_size, rnn_type=nn.LSTM, bidirectional=False, batch_norm=True, dropout=0.):
...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, n_layers=1,
        ...
        rnns = []
        rnn = BatchRNN(input_size=rnn_input_dims, hidden_size=self.hidden_size, rnn_type=nn.LSTM,
                       bidirectional=self.bidirectional, batch_norm=False, dropout=0.)
        rnns.append(('0', rnn))
        for x in range(n_layers - 1):
            rnn = BatchRNN(input_size=self.hidden_size, hidden_size=self.hidden_size, rnn_type=nn.LSTM,
        ...
        self.fc = SequenceWise(
            nn.Sequential(
                ...
                nn.Dropout(0.5),
                nn.Linear(self.hidden_size, VOC_SIZE + 2, bias=False)
            )
        )

    def forward(self, input_var, input_lengths):
        output_lengths = self.get_seq_lens(input_lengths)
        x, _ = self.conv(input_var, output_lengths)
        ...
        return x.transpose(0, 1), None, self.fc(x)  # T N C >
...
class DecoderRNN(nn.Module):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, context, attn_w):
        if self.training:
            self.rnn.flatten_parameters()
...
class Seq2Seq(nn.Module):
    def __init__(self, vocab_size):
        ...
        self.encoder = EncoderRNN(
            input_size=40, hidden_size=768, n_layers=3, input_dropout_p=0.0, dropout_p=0.3,
            bidirectional=True, rnn_cell='lstm', variable_lengths=False
        )
        self.decoder = DecoderRNN(
            vocab_size=vocab_size, max_len=80, hidden_size=512, encoder_size=768,
            sos_id=VOC_SIZE, eos_id=VOC_SIZE + 1, n_layers=2, rnn_cell='lstm',
            bidirectional_encoder=True, bidirectional_decoder=False,
            dropout_p=0.3, use_attention=True
        )

        # ctc loss module
        self.ctc_loss = nn.CTCLoss(blank=VOC_SIZE - 1)  # vocab_size - 1 : blank '#'
    ...
    def forward(self, input_variable, input_lengths=None, chr_target=None, chr_lengths=None,
                teacher_forcing_ratio=0):
        encoder_outputs, _, ctc_logits = self.encoder(input_variable, input_lengths)

        self.decoder.rnn.flatten_parameters()
        decoder_output = self.decoder(inputs=chr_target,
                                      encoder_hidden=None,
                                      encoder_outputs=encoder_outputs,
                                      teacher_forcing_ratio=teacher_forcing_ratio)

        if chr_target is not None and chr_lengths is not None:
            ctc_loss = self.calculate_ctc_loss(ctc_logits, chr_target, chr_lengths)
            return decoder_output, ctc_logits, ctc_loss
        else:
            return decoder_output, ctc_logits, None
...
if __name__ == '__main__':
    ...
    args.add_argument("--learning_anneal", type=float, default=1.01)
    ...
    args.add_argument("--inf_encoder", type=bool, default=True)
    ...
    inf_encoder = config.inf_encoder

    # create model
    model = Seq2Seq(vocab_size=VOC_SIZE + 2)  # + sos eos

    # make criterion
    criterion = nn.CrossEntropyLoss(reduction='mean')

    # mfcc function
    mfcc_func = MFCC(n_mfcc=40, log_mels=True, melkwargs={
        'n_fft': 400, 'hop_length': 200, 'n_mels': 40, 'f_min': 60
    })
    ...
    if mode == 'train':
        ...
        for epoch in range(num_epochs):
            ...
            for iter_, data in enumerate(train_dataloader):
                ...
                # update weight
                logit, _, ctc_loss = model(mfcc, input_lengths=mfcc_mask.sum(-1).long(), chr_target=txt_arr,
                                           chr_lengths=txt_mask.sum(-1).long(), teacher_forcing_ratio=0.5)
                logit = torch.stack(logit, dim=1)
                y_hat = logit.max(-1)[1]

                ce_loss = criterion(logit.contiguous().view(-1, logit.size(-1)), target.contiguous().view(-1))
                loss = ce_loss * 0.5 + ctc_loss * 0.5
                ...
                if (iter_ + 1) % print_iter == 0:
                    ...
                    print('[{:.3f}/{:d}] loss({}) ce loss ({}) ctc loss ({}) '
                          'elapsed {} expected per epoch {}'.format(
                        _epoch, num_epochs, loss.item(), ce_loss.item(), ctc_loss.item(), elapsed, expected),
                        flush=True)
                    time_ = datetime.datetime.now()
            ...
            train_ctc_loss /= len(train_dataloader)
            ...
            for iter_, data in enumerate(validate_dataloader):
                ...
                with torch.no_grad():
                    ...
                    # forward
                    logit, _, ctc_loss = model(mfcc, input_lengths=mfcc_mask.sum(-1).long(), chr_target=txt_arr,
                                               chr_lengths=txt_mask.sum(-1).long(), teacher_forcing_ratio=0.)
                    logit = torch.stack(logit, dim=1)
                    y_hat = logit.max(-1)[1]
                    ...
                    valid_ctc_loss += ctc_loss.item()

            valid_loss /= len(validate_dataloader)
            valid_ce_loss /= len(validate_dataloader)
            valid_ctc_loss /= len(validate_dataloader)
            ...
            time_ = datetime.datetime.now()
            elapsed = datetime.datetime.now() - time_
            print('[epoch {}] elapsed: {}, train : loss {} / ce {} / ctc {}, / valid : loss {} / ce {} / ctc {}'.format(
                epoch + 1, elapsed, train_loss, train_ce_loss, train_ctc_loss, valid_loss, valid_ce_loss, valid_ctc_loss
            ), flush=True)
    ...
    #
        ...
        for iter_, data in enumerate(test_dataloader):
            ...
            with torch.no_grad():
                ...
                logit, ctc_logits, _ = model(mfcc, input_lengths=mfcc_mask.sum(-1).long(), teacher_forcing_ratio=0.)
                if inf_encoder:
